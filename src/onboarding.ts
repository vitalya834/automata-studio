...
export const onboardingTemplates: readonly OnboardingTemplate[] = [
  ...
  },
  {
    id: 'rest-api',
    title: 'REST API',
    subtitle: 'Жизненный цикл заказа в веб-API',
    ...
    what: 'Поведение REST-сервиса: заказ создаётся, оплачивается, отгружается или отменяется в правильном порядке.',
    states: 'empty, created, paid, shipped, cancelled (модель RestOrderWorkflow)',
    inputs: 'create, pay, ship, cancel → HTTP-запросы из конфигурации адаптера',
    outputs: 'created_201, paid_200, shipped_200, cancelled_200 — статусы и тела ответов',
  ...
  },
  {
    ...
    what: 'Поведение контроллера двигателя: запуск, останов, аварийный trip и сброс через регистры FC1–FC6.',
    states: 'ready, running, fault (модель ModbusMotorController)',
    inputs: 'start, stop, trip, reset → типизированные Modbus-операции из конфигурации',
    outputs: 'running, stopped, fault, ready — предикаты над прочитанными значениями',
  ...
  },
  {
    ...
    what: 'Поведенческий контракт конвейера инференса: валидация запроса, выполнение и выдача результата.',
    states: 'idle, validating, inferencing, completed, rejected (модель MlInferencePipeline)',
    inputs: 'submit_valid, submit_invalid, validation_ok, inference_ok → HTTP-запросы адаптера',
    outputs: 'accepted, rejected, running, prediction — метки из JSON-ответа (JSON Pointer)',
