...
- «Open example» загружает игровую модель в DSL-редактор, а временной
  контроллер — в Timed Testing Workbench прямо в браузере;
- сценарии для Node runner (HTTP, Modbus, ML, CLI) показывают реальные
  команды `npm run` и ссылки на существующие примеры и документацию, а не
  имитируют выполнение, недоступное браузеру;
- пятишаговый путь пользователя: выбрать шаблон → изучить граф →
  сгенерировать тесты → запустить адаптер → изучить отчёт;
